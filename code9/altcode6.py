...
def get_patent_details(google_url):
    try:
        ...
        if response.status_code == 200:
            ...
            return pdf_link, title, inventor, publication_date, abstract
        return "PDF not found", "Title not found", "Inventor not found", "Publication date not found", "Abstract not found"
...
def export_to_excel():
    ...
    # Set column widths
    column_widths = [20, 20, 20, 25, 15, 15, 15, 15, 70, 50]
    for col, width in enumerate(column_widths):
        worksheet.set_column(col, col, width)

    # Write headers
    headers = ["Ref No.", "Inventor", "Title", "Publication Date", "PDF_LINK", "Google Link", "Espacenet Link", "USPTO Link", "ABSTRACT", "NOTES"]
    ...
    for number in patent_numbers:
        ...
        pdf_link, title, inventor, publication_date, abstract = get_patent_details(google_url)
        ...
        worksheet.write(row, 4, pdf_link)
        worksheet.write_url(row, 5, google_url, link_format, string=cleaned_number)
        worksheet.write_url(row, 6, espacenet_url, link_format, string=cleaned_number)
        worksheet.write_url(row, 7, uspto_url, link_format, string=cleaned_number)
        worksheet.write(row, 8, abstract, abstract_format)
...
#wrapper > div:nth-child(3) > div.flex-2.style-scope.patent-result > section > header > div > a