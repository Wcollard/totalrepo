...
def export_to_excel():
    ...
    # Set column widths
    column_widths = [20, 25, 20, 20, 20, 20, 20, 70, 50]
    for col, width in enumerate(column_widths):
        worksheet.set_column(col, col, width)

    # Write headers
    headers = ["Ref No.", "Title", "Inventor", "Publication Date", "Google Link", "Espacenet Link", "USPTO Link", "ABSTRACT", "NOTES"]
    ...
    for number in patent_numbers:
        ...
        worksheet.write(row, 1, title, abstract_format)
        worksheet.write(row, 2, inventor)
