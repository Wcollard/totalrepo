...
def save_to_database(patent_info):
    ...
    conn.commit()
    conn.close()
    # ... [your imports and previous code] ...
...
def fetch_patents():
    ...
    for patent in patent_numbers:
        try:
            if check_patent_exists(patent):
                ...
                conn.close()
                if result is None:
                    print(f"Patent {patent} not found in DB.")
                    continue
                patent_info = {
    ...
    if data:
        ...
        print("No patent data found to export.")
        messagebox.showwarning("No Data", "No patent data found to export.")
