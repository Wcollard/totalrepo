...
class PatentScraperApp:
    ...
    def scrape_patent(self):
        ...
        try:
            ...
            self.log_message(f"Assignee: {patent_data.get('assignee', 'Not found')}")
    ...
    def extract_patent_data(self, soup):
        ...
        # Extract assignee
        data['assignee'] = self.extract_assignee(soup)
    ...
    def save_to_excel(self, patent_num, url, patent_data):
        ...
        ws[f'F{next_row}'] = patent_data.get('assignee', 'Not found')
        ws[f'G{next_row}'] = patent_data.get('abstract', 'Not found')
        ws[f'H{next_row}'] = time.strftime('%Y-%m-%d %H:%M:%S')
        ...
        ws.column_dimensions['F'].width = 30  # Assignee
        ws.column_dimensions['G'].width = 60  # Abstract
        ws.column_dimensions['H'].width = 20  # Date Scraped
        
        # Wrap text in title, inventor, assignee, and abstract columns
        wrap_columns = ['B', 'E', 'F', 'G']
